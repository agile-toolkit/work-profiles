...
- **Cross-app integration pattern**: this app is the *source of truth* for team composition/capacity — it publishes `wp-profiles-export` and `work-profiles:lastSession` for other apps to read, and separately *reads* `improvement-board-items` (one-way, no write-back). Several open issues extend this pattern further — see `ROADMAP.md`.
